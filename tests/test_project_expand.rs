...
fn test_project_expand() {
    ...
    assert!(
        lib_path.exists(),
        "Library should exist at {lib_path:?}"
    );
    ...
    assert!(stdout.contains("Hello, World!"), "Output should contain 'Hello, World!'");
    assert!(stdout.contains("Bonjour"), "Output should contain 'Bonjour'");
}
