...
first we have a description of messages in different languages, in a yaml file. This will generate C files, and we have our `main.c` that will print some messages

file `greetings.yml`

```yml
{{#include ../../demo_projects/demo_cluster/greetings.yml}}
```
...
for each node, we have an `expand` trait. It usually does nothing, but, in our case, `exand` will :
